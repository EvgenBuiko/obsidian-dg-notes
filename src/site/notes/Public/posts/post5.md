---
{"dg-publish":true,"permalink":"/public/posts/post5/","title":"Но что же эти ваши асинхронность и корутины такое под капотом? [[post5|Read]]","tags":["blog","Dev","C","Cplusplus"]}
---

По сути весь пост интересная историческая справка. В посте будет рассмотрена стековая реализация. Впринципе остальные реализации будут отличаться по сути только методом хранения данных.
...
```c
#define DECLARE() static int state
#define INIT() state = 0
...
#define END }
```
Как можно заметить сейчас yield что то довольно знакомое по своему имени)

</div></div>


И вот у нас получился синтаксис генераторов. На деле они и являются основным механизмом корутин(асинхронности) - механизм прерывания с сохранением состояния. 

Далее рассматривать не вижу смысла ибо остальная часть реализации не замысловатая и варируется от языка к языку. Подробнее можете на это посмотреть в инете загуглив "yield асинхронность". Там вы встретите примеры ака "строим свою асинхронность генераторами на <ваш лучший язык программировани>" где генераторы заворачивают в task которые обрабатываются в event loop и вот получается асинхронная система
